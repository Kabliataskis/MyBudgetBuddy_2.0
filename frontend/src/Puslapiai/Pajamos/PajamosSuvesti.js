...
s
	return (
        <>
            <button onClick={() => setModal_PajamosSuvesti(true)} >Pridėti pajamas</button>
            ...
            <div className="Pajamos-modal-container" onMouseDown={onMouseDown}>
                <div className="modal-content">
                        ...
                        <span className="modal-close-btn" onClick={() => closeModal()}><AiOutlineClose/></span>
                    <form className="Pajamos-modal-form" onSubmit={handleSubmit}>
                        <label htmlFor="title">Pavadinimas</label>
                        <p><input type="text" name="title" id="title" placeholder="Pavadinimas" maxLength="20"  required onChange={handleChange} value={title}/></p>

                        <label htmlFor="date">Data</label>
                        <p><input type="datetime-local" name='date' id="date" required onChange={handleChange} value={date}/></p>
                        
                        <label htmlFor="title">Suma</label>
                        <p className='eur'><input type="number" name="amount" id="amount" placeholder="Suma" min="0.01" max={max_amount} step="0.01" autoComplete='off' required onChange={handleChange} value={amount}/></p>

                        <button className="add-btn" type="submit">Pridėti</button>
                        <button className="cancel-btn" type="reset" onClick={() => closeModal()} >Atšaukti</button>
                    </form>
                </div>
            </div>
            : null}
